<script lang="ts">
	...
	let selectedJob = $derived(jobs.find((j: Job) => j.id === selectedJobId) ?? null);
	...
	async function handleDeleteJob(jobId: string) {
		if (!confirm('Diesen Request wirklich loeschen?')) return;
		try {
	...
	async function handleClearAll() {
		if (!confirm('Gesamte Request-Historie wirklich loeschen?')) return;
		try {
...
<svelte:head>
	<title>Requests | Actindo Middleware</title>
</svelte:head>

<PageHeader title="Requests" subtitle="Vergangene API-Requests und Fehler">
	{#snippet actions()}
		{#if perms.canWrite}
...
<div class="grid lg:grid-cols-2 gap-6">
	...
	<Card>
		...
		{:else}
			...
			<div class="flex items-center justify-between mt-4 pt-4 border-t border-white/10">
				<span class="text-sm text-gray-400">Seite {page} / {totalPages}</span>
	...
	<Card>
		...
		{:else}
			<div class="text-center py-12 text-gray-400">
				<Activity size={48} class="mx-auto mb-4 opacity-50" />
				<p>Waehle einen Request aus der Liste</p>
			</div>
		{/if}
	</Card>
</div>
...
<!-- Replay Modal -->
<Modal bind:open={replayOpen} title="Request Replay">
	<div class="space-y-4">
		<p class="text-sm text-gray-400">Bearbeite das Request-Payload und sende den Request erneut.</p>
		{#if replayError}
